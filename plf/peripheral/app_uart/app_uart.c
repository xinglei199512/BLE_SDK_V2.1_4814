...
static void uart_rx_complete(app_uart_inst_t *inst)
{
    ...
    {
        periph_unlock(&inst->env.rx_lock);
...
static void uart_tx_complete(app_uart_inst_t *inst)
{
    ...
    periph_unlock(&inst->env.tx_lock);
    // Call handler
...
periph_err_t uart_reinit(periph_inst_handle_t hdl)
{
...
periph_err_t app_uart_init(periph_inst_handle_t hdl)
{
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    ...
    return uart_reinit(hdl);
}

periph_err_t app_uart_uninit(periph_inst_handle_t hdl)
{
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    if(periph_lock_state_get(&inst->env.tx_lock)||periph_lock_state_get(&inst->env.rx_lock))
    {
        return PERIPH_BUSY;
    }
    ...
    app_uart_comm_params_t *param = &inst->param;
    uart_universal_func.pin_cfg_func(inst,param->rx_pin_no,UART_RXD_PIN,false);
    uart_universal_func.pin_cfg_func(inst,param->tx_pin_no,UART_TXD_PIN,false);
    if(param->flow_control_en)
    {
        uart_universal_func.pin_cfg_func(inst,param->rts_pin_no,UART_RTS_PIN,false);
        uart_universal_func.pin_cfg_func(inst,param->cts_pin_no,UART_CTS_PIN,false);
    }
    return PERIPH_NO_ERROR;
}
...
periph_err_t app_uart_read(periph_inst_handle_t hdl,uint8_t *bufptr, uint32_t size, void (*callback) (void*,uint8_t),void* dummy)
{
    ...
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    if(periph_lock(&inst->env.rx_lock)==false)
    {
        return PERIPH_BUSY;
    }
...
periph_err_t app_uart_read_cancel(periph_inst_handle_t hdl,uint32_t *remaining_length,void **dummy)
{
    ...
    uart_universal_func.intr_op_func(inst,INTR_DISABLE);
    if(periph_lock_state_get(&inst->env.rx_lock)==false)
    {
...
periph_err_t app_uart_write(periph_inst_handle_t hdl,uint8_t *bufptr, uint32_t size, void (*callback) (void*,uint8_t),void* dummy)
{
    ...
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    if(periph_lock(&inst->env.tx_lock)==false)
    {
        return PERIPH_BUSY;
    }
