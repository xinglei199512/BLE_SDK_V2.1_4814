...
struct
{
    void (*spim_callback)(uint8_t,uint8_t);
    void (*spis_callback)(uint8_t,uint8_t);
}test_complete;
static void spis_transfer_cb(void *cb_param)
{
    if(memcmp(spim_tx_data,spis_rx_buf,sizeof(uint32_t)*DATA_LENGTH)==0)
    {
        ...
        test_complete.spis_callback(0,0);
    }else
    {
        LOG(LOG_LVL_INFO,"spis rx spim tx fail\n");
        test_complete.spis_callback(0,1);
    }
}
...
static void spim_transfer_cb(void *cb_param)
{
    if(memcmp(spis_tx_data,spim_rx_buf,sizeof(uint32_t)*DATA_LENGTH)==0)
    {
        ...
        test_complete.spim_callback(0,0);
    }else
    {
        LOG(LOG_LVL_INFO,"spim rx spis tx fail\n");
        test_complete.spim_callback(0,1);
    }
}
...
{
    test_complete.spim_callback = spim_cb;
    test_complete.spis_callback = spis_cb;
    uint8_t i;
