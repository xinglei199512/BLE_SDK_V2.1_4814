...
static uint8_t app_iic_eeprom_test(app_iic_inst_t *handle , uint8_t use_dma , uint8_t speed_mode)
{
	...
	for(page_index = 0;page_index < AT24C16_SIZE / AT24C16_PAGE_SIZE ; page_index ++)
	{
        ...
        #ifdef DEBUG_ERROR_DEVADDR
            //debug for error device address
            if(page_index % 2) device_address = 0x12;
        #endif
		...
		//write
		err_w = app_iic_write(&handle->inst , iic_tx_buf , AT24C16_WRITE_SIZE , device_address , mem_address , app_iic_tx_finish , 0);
		while(iic_tx_ok == 0);
		app_iic_delay_ms(5);
		//read
        err_r = app_iic_read(&handle->inst , iic_rx_buf , AT24C16_WRITE_SIZE , device_address , mem_address , app_iic_rx_finish , 0);
		while(iic_rx_ok == 0);
		app_iic_delay_ms(1);
		...
		{
			LOG(LOG_LVL_INFO,"page%d,ERROR\n",page_index);
			success = 0;
		}
		else
		{
            #ifdef DEBUG_ERROR_DEVADDR
                LOG(LOG_LVL_INFO,"page%d,OK\n",page_index);
            #else
                if((page_index % 32)== 0)   LOG(3,"page%d,OK\n",page_index);
            #endif
		}
	}
