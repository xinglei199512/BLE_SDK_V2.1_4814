...
static void iic_stop_det_isr(app_iic_inst_t *inst)
{
    ...
    //valid stop bit judge
    if(inst->param.use_dma)
    {
        //dma mode have not change tx.remain.
        valid_stop_det = 1;
    }
    else
    {
        ...
        if((app_iic_get_tfe(inst->reg)) && (inst->env.tx.remain <= 1))
        {
            valid_stop_det = 1;
        }
        else
        {
            ...
            if(inst->env.errordetect == 1)
            {
                valid_stop_det = 1;
            }
            else
            {
                LOG(LOG_LVL_WARN , "invald iic stop bit\n");
            }
        }
    }
...
periph_err_t iic_reinit(periph_inst_handle_t hdl)
{
    //get parameter
...
periph_err_t app_iic_init(periph_inst_handle_t hdl)
{
    //get parameter
    app_iic_inst_t *inst = CONTAINER_OF(hdl, app_iic_inst_t, inst);
    ...
    return iic_reinit(hdl);
}

periph_err_t app_iic_uninit(periph_inst_handle_t hdl)
{
    app_iic_inst_t *inst = CONTAINER_OF(hdl, app_iic_inst_t, inst);
    reg_iic_t *reg = inst->reg;
    if(periph_lock_state_get(&inst->env.iic_lock))
    {
        return PERIPH_BUSY;
    }
...
periph_err_t app_iic_read(periph_inst_handle_t hdl,uint8_t *bufptr, uint32_t size , uint16_t device_address, uint16_t mem_address , void (*callback) (void*,uint8_t),void* dummy)
{
    ...
    iic_clear_all_irq(reg , 1 << IIC_INTR_STOP_DET);//clear stop dec IRQ for glitch
    if(inst->param.use_dma)
    {
