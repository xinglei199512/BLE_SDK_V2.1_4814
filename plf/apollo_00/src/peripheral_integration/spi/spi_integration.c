...
static void spi_pin_cfg(app_spi_inst_t * inst,uint8_t pin_num,uint32_t pin_role,bool enable)
{
    ...
    case SPIM_CLK_PIN:
        ...
        if(idx)
        {
            sysc_cpu_spim1_en_setf(enable);
        }else
        {
            sysc_cpu_spim0_en_setf(enable);
        }
    break;
    case SPIS_CLK_PIN:
        if(enable)
...
sysctrl_io_config(pin_num, GPIO_DISABLED);
    ...
    case SPIM_CS1_PIN:
        ...
        if(idx)
        {
            sysc_cpu_spim1_cs1_en_setf(enable);
        }else
        {
            sysc_cpu_spim0_cs1_en_setf(enable);
        }  
    break;
    default:
        BX_ASSERT(0);
...
{
    uint8_t idx = inst->idx;
    uint8_t periph_idx = inst->slave? PERIPH_DOMAIN_SPIS : PERIPH_DOMAIN_SPIM0+idx;
    switch(sys_stat)
    {
    case SPI_INIT:
        recovery_list_add(periph_domain_recovery_buf,periph_idx, &inst->inst);
    break;
    case SPI_UNINIT:
        recovery_list_remove(periph_domain_recovery_buf,periph_idx);
    break;
    case SPI_TX_START:
    case SPI_RX_START:
    case SPI_TXRX_START:
