...
N_XIP_SECTION static void modem_vcocali_value_restore()
{
    ...
    {
        void ble_mdm_vcocali_capdev_1_setf(uint16_t vcocali_capdev_1) N_XIP_SECTION;
        ble_mdm_vcocali_capdev_1_setf(vco_calibration.vco_capdev[0]|((vco_calibration.vco_capdev[0]*2)<<6));
        void ble_mdm_vcocali_capdev_2_setf(uint16_t vcocali_capdev_2) N_XIP_SECTION;
        ble_mdm_vcocali_capdev_2_setf(vco_calibration.vco_capdev[1]|((vco_calibration.vco_capdev[1]*2)<<6));
        void ble_mdm_vcocali_capdev_3_setf(uint16_t vcocali_capdev_3) N_XIP_SECTION;
        ble_mdm_vcocali_capdev_3_setf(vco_calibration.vco_capdev[2]|((vco_calibration.vco_capdev[2]*2)<<6));
        void ble_mdm_vcocali_capdev_4_setf(uint16_t vcocali_capdev_4) N_XIP_SECTION;
        ble_mdm_vcocali_capdev_4_setf(vco_calibration.vco_capdev[3]|((vco_calibration.vco_capdev[3]*2)<<6));
...
N_XIP_SECTION void modem_init()
{
    ...
    void ble_mdm_vco_stb_time_setf(uint8_t vco_stb_time) N_XIP_SECTION;
...
//        BX_ASSERT(0);
    ...
    void ble_mdm_vco_cnt_spacing_setf(uint8_t vco_cnt_spacing) N_XIP_SECTION;
...
#elif (RF_PARAM == 2 || RF_PARAM == 3) && (AUTO_CAL == 0)
...
{
#if !((defined MODEM_DYNAMIC_DERIV_CALIB) && (MODEM_DYNAMIC_DERIV_CALIB == 1))
    if(param2 != VCO_VALUE_INVALID)        vco_save_aa55[1] = param2;
#endif    
    if(param3 != VCO_VALUE_INVALID)        vco_save_aa55[2] = param3;
#if !((defined MODEM_DYNAMIC_DERIV_CALIB) && (MODEM_DYNAMIC_DERIV_CALIB == 1))    
    if(param4 != VCO_VALUE_INVALID)        vco_save_aa55[3] = param4;
#endif    
}
void set_vco_buff_others(uint32_t param2 , uint32_t param3 , uint32_t param4)
{
#if !((defined MODEM_DYNAMIC_DERIV_CALIB) && (MODEM_DYNAMIC_DERIV_CALIB == 1))
    if(param2 != VCO_VALUE_INVALID)        vco_save_others[1] = param2;
#endif    
    if(param3 != VCO_VALUE_INVALID)        vco_save_others[2] = param3;
#if !((defined MODEM_DYNAMIC_DERIV_CALIB) && (MODEM_DYNAMIC_DERIV_CALIB == 1))
    if(param4 != VCO_VALUE_INVALID)        vco_save_others[3] = param4;
#endif    
}
...
/*        LOG(LOG_LVL_INFO,"%2d:%02d %02d %02d %02d %02d %02d %02d %02d\n",i,
                (ble_mdm_vcocali_capdev_1_getf()>>6)&0x3f,
                (ble_mdm_vcocali_capdev_2_getf()>>6)&0x3f,
                (ble_mdm_vcocali_capdev_3_getf()>>6)&0x3f,
                (ble_mdm_vcocali_capdev_4_getf()>>6)&0x3f,
                ble_mdm_vcocali_capdev_1_getf()&0x3f,
                ble_mdm_vcocali_capdev_2_getf()&0x3f,
                ble_mdm_vcocali_capdev_3_getf()&0x3f,
                ble_mdm_vcocali_capdev_4_getf()&0x3f
                );*/
...
//    LOG(LOG_LVL_INFO,"\n");
//    LOG(LOG_LVL_INFO,"before minus 0.23:\n");
//    LOG(LOG_LVL_INFO,"  :%02d %02d %02d %02d %02d %02d %02d %02d\n\n",/*???10??*/
/*            calib_val.band0_2M/VCO_CALIB_NUM,
            calib_val.band1_2M/VCO_CALIB_NUM,
            calib_val.band2_2M/VCO_CALIB_NUM,
            calib_val.band3_2M/VCO_CALIB_NUM,
            calib_val.band0_1M/VCO_CALIB_NUM,
            calib_val.band1_1M/VCO_CALIB_NUM,
            calib_val.band2_1M/VCO_CALIB_NUM,
            calib_val.band3_1M/VCO_CALIB_NUM
            );*/
    ...
    LOG(LOG_LVL_INFO,"after minus 0.23:\n");
    LOG(LOG_LVL_INFO,"  :%02d %02d %02d %02d %02d %02d %02d %02d\n\n",
            calib_val.band0_2M,
            calib_val.band1_2M,
            calib_val.band2_2M,
...
//    LOG(LOG_LVL_INFO,"  :%02x %02x %02x %02x %02x %02x %02x %02x\n\n",
//            calib_val_band[0][0]/VCO_CALIB_NUM,
//            calib_val_band[0][1]/VCO_CALIB_NUM,
//            calib_val_band[0][2]/VCO_CALIB_NUM,
//            calib_val_band[0][3]/VCO_CALIB_NUM,
//            calib_val_band[1][0]/VCO_CALIB_NUM,
//            calib_val_band[1][1]/VCO_CALIB_NUM,
//            calib_val_band[1][2]/VCO_CALIB_NUM,
//            calib_val_band[1][3]/VCO_CALIB_NUM
//            );

//    uint16_t deriv_calib_band0 = (uint16_t)(calib_val_band[1][0]/VCO_CALIB_NUM);
    ...
    /*vco_save_others[0] = vco_save_aa55[0] = ((calib_val.band0_1M*2)<<6) | calib_val.band0_1M;
    vco_save_others[1] = vco_save_aa55[1] = ((calib_val.band1_1M*2)<<6) | calib_val.band1_1M;
    vco_save_others[2] = vco_save_aa55[2] = ((calib_val.band2_1M*2)<<6) | calib_val.band2_1M;
    vco_save_others[3] = vco_save_aa55[3] = ((calib_val.band3_1M*2)<<6) | calib_val.band3_1M;
	*/
    enable_vco_value(false);
    /*LOG(LOG_LVL_INFO,"  :%03x %03x %03x %03x \n",
            vco_save_others[0],
            vco_save_others[1],
            vco_save_others[2],
            vco_save_others[3]
            );
    LOG(LOG_LVL_INFO,"(2M value = 1M value * 2)\n\n");
    */
//    vco_save_aa55[0] = vco_save_others[0] + 2;
//#if ((defined MODEM_DYNAMIC_DERIV_CALIB) && (MODEM_DYNAMIC_DERIV_CALIB == 1))
//    uint16_t deriv_calib_band1 = (uint16_t)(calib_val_band_1/VCO_CALIB_NUM);
