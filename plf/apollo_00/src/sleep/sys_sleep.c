...
{
    #if HW_BX_VERSION == 00
    N_XIP_SECTION void sysc_awo_blelp_inrp_clr_set(uint32_t value);
...
{
    #if HW_BX_VERSION == 00
    N_XIP_SECTION void sysc_awo_blelp_inrp_en_setf(uint8_t blelp_inrp_en);
...
{
    void sysc_awo_blelp_inrp_en_setf(uint8_t blelp_inrp_en) N_XIP_SECTION;
    #if HW_BX_VERSION == 00
...
N_XIP_SECTION void BLE_LP_IRQHandler(void)
{
    ...
    void NVIC_EnableIRQ(IRQn_Type IRQn) N_XIP_SECTION;
    NVIC_EnableIRQ(BLE_MAC_IRQn);
    uint32_t NVIC_GetPendingIRQ(IRQn_Type IRQn) N_XIP_SECTION;
...
//}
void switch_to_32m_xtal(uint32_t current_time, uint8_t mode)
{
    if(mode == 0)
    {
        wait_rtc_until(current_time+ XTAL_STARTUP_TIME_50);
        reg_set_xtal_current_below_temp0();
        wait_rtc_until(current_time+ XTAL_STARTUP_TIME_50 + 1);
        sysc_awo_o_ana_16m_selrc_setf(0);
    }
    else
    {
