...
{
    light_lightness_client_t *client = GET_SERVER_MODEL_PTR(light_lightness_client_t, model);
    LOG(3, "light_lightness_status_rx\n");

    if(client->get_current_state_cb) {
        uint8_t status = client->get_current_state_cb();

        if(status == WAIT_FOR_LIGHT_LIGHTNESS_STATUS) {
            ...
            if(payload_size == sizeof(light_lightness_actual_default_status_t))
                present_value = *(uint16_t *)(access + 2);
            else if(payload_size == sizeof(light_lightness_actual_status_t))
...
void light_lightness_msg_publish(light_lightness_client_t *client,void* msg, uint32_t msgLen, uint16_t dst_addr, uint32_t opcode)
{
    ...
    LOG(3,"light_lightness_msg_publish\n");
    ...
    tx_param.akf = 1;
    access_pdu_tx_t * ptr = access_model_pkt_build_fill(&tx_param,light_lightness_msg_publish_done,(uint8_t *)msg);
    BX_ASSERT(ptr);
    access_send(ptr);
}
