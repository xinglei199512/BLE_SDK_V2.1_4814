...
N_XIP_SECTION static void lld_sleep_compensate(void)
{
    ...
    N_XIP_SECTION void ble_basetimecntcorr_set(uint32_t);
    N_XIP_SECTION void ble_finecntcorr_set(uint32_t);
    ...
    #if HW_BX_VERSION == 00
     N_XIP_SECTION void sysc_awo_awo_reset_signal_set(uint32_t);
    sysc_awo_awo_reset_signal_set(SRST_BLE_LP_CLR);
    #endif
    N_XIP_SECTION void ble_deep_sleep_corr_en_setf(uint8_t);
...
{
    N_XIP_SECTION void ble_extwkupdsb_setf(uint8_t);
    ...
    N_XIP_SECTION void ble_intack_clear(uint32_t value);
    // Clear slot interrupt
    ble_intack_clear(BLE_CSCNTINTACK_BIT);

    N_XIP_SECTION void ble_intcntl_set(uint32_t value);
